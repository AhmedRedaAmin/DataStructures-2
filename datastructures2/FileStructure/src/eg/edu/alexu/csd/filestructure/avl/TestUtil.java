...
public class TestUtil<T extends Comparable<T>> {
    ...
    public boolean validateAVL(final INode<T> root) {
        return validateBST(root) && checkBalance(root);
    }

    public boolean validateBST(final INode<T> root) {
        if (root == null) {
            return true;
        }
    ...
    private void inOrderTraversal(final INode<T> node,
    ...
    public boolean checkBalance(final INode<T> root) {
        return checkBalanceAux(root) != -1;
    }

    private int checkBalanceAux(final INode<T> node) {
        if (node == null) {
            return 0;
        }
